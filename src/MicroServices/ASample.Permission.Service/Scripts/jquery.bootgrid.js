...
;(function ($, window, undefined)
{
    ...
    function init()
    {
        ...
        //renderSearchField.call(this); //commented by shen for nor render searchField 2016-07-21
    ...
    */
    ...
    function loadData()
    {
        ...
        if (this.options.ajax)
        {
            ...
            var settings = {
                ...
                {
                    if (that.xqr.responseText.indexOf(".html") != -1)
                        window.parent.location.href = "../../" + that.xqr.responseText;
    ...
    function renderColumnSelection(actions)
    {
        if (this.options.columnSelection && this.columns.length > 1)
        {
            ...
            {
                //modified by shen 2016-07-09 change if (column.visibleInSelection) to if (column.visibleInSelection && column.visible)
                //fixed bug right dropdown colunm list show not visible column
                if (column.visibleInSelection && column.visible)
                {
        ...
        if (this.options.navigation !== 0)
        {
            ...
            if (this.rowCount !== -1 && paginationItems.length > 0)
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    //modified by shen 2016-03-05
                    if (pos == current) {
                        renderPaginationItem.call(this, pagination, pos, pos, "page-" + pos + " active")
                              ._bgEnableAria()._bgSelectAria(pos == current);//=== modified to ==
                    } else {
                        renderPaginationItem.call(this, pagination, pos, pos, "page-" + pos)
                              ._bgEnableAria()._bgSelectAria(pos === current);
                    }
        ...
        var that = this,
            ...
            item = $(tpl.paginationItem.resolve(values))
                .on("click" + namespace, getCssSelector(css.paginationButton), function (e)
                {
                    ...
                    {
                        //Modified by shen 2016-06-15 ,修改next属性计算公式
                        /*var commandList = {
                            first: 1,
                            prev: that.current - 1,
                        ...
                        var nextPage = parseInt(that.current) + 1 <= that.totalPages ? parseInt(that.current) + 1 : 1;
                        var commandList = {
                            first: 1,
                            prev: that.current - 1,
                            next: nextPage,
    ...
    function renderRows(rows)
    {
        if (rows.length > 0)
        {
            ...
            //added by shen 2016-03-09
            registerDblRowEvents.call(this, tbody);
        }
        else
        {
    ...
    /* 注册双击事件 added by shen 2016-03-09 */
    function registerDblRowEvents(tbody) {
        var that = this,
            selectBoxSelector = getCssSelector(this.options.css.selectBox);

        if (this.selection) {
            tbody.off("dblclick" + namespace, selectBoxSelector)
                .on("dblclick" + namespace, selectBoxSelector, function (e) {
                    e.stopPropagation();
        ...
        tbody.off("dblclick" + namespace, "> tr")
            .on("dblclick" + namespace, "> tr", function (e) {
                e.stopPropagation();
    ...
    Grid.defaults = {
        ...
        labels: {
            all: "全部",
            infos: "显示 {{ctx.start}} 到 {{ctx.end}} 行，共 {{ctx.total}} 行",
            loading: "加载中...",
            noResults: "查询数据为空！",
            refresh: "刷新",
            search: "搜索"
    ...
    /**
    * addrow only in UI
    * added by shen 2016-01-27
    **/
    Grid.prototype.addRows = function (rows) {
        var appendedRows = [];
        var thatAjax = false;
        if (this.options.ajax) {
            thatAjax = this.options.ajax;
        ...
        //appendedRows = appendedRows.concat(this.currentRows);
        ...
        if(thatAjax) this.options.ajax = thatAjax;
    ...
    {
        //Modiufied by shen 2016-01-23
        //this.current = (current !== 'undefined') ? current : 1;//1; // reset
        loadData.call(this);
        ...
        if (this.identifier != null)
        {
            ...
            else
            {
                ...
                for (var i = 0; i < rowIds.length; i++)
                {
                    ...
                    {
                        //modified by shen 2016-05-11 alter === to ==
        ...
        if (this.selection)
        {
            ...
            while (rowIds.length > 0 && !(!this.options.multiSelect && selectedRows.length === 1))
            {
                ...
                if ($.inArray(id, this.selectedRows) === -1)
                {
                    ...
                    {
                        //modified by shen 2016-05-06 change === to ==
